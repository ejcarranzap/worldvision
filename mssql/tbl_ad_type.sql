...
IF OBJECT_ID('fn_seq_tbl_ad_type') IS NOT NULL DROP FUNCTION fn_seq_tbl_ad_type
IF OBJECT_ID('sp_tbl_ad_type') IS NOT NULL DROP PROCEDURE sp_tbl_ad_type
GO
CREATE FUNCTION fn_seq_tbl_ad_type()
  RETURNS INT
AS
BEGIN
  DECLARE @ID INT = 0
  SELECT @ID = ISNULL(MAX(Id_type),0)+1 FROM tbl_ad_type
  RETURN @ID
END
GO
CREATE PROCEDURE sp_tbl_ad_type
...
SET NOCOUNT ON
  ...
  BEGIN 
    BEGIN TRY
    BEGIN TRAN
      SET IDENTITY_INSERT tbl_ad_type ON
      
      SET @Id_type = dbo.fn_seq_tbl_ad_type()
      INSERT INTO tbl_ad_type(
      Id_type,
      code,
      name,
      date_mod,
      ...
      SELECT A.* FROM tbl_ad_type A 
      WHERE A.Id_type = @IDENTITY 
      SET IDENTITY_INSERT tbl_ad_type OFF
    COMMIT
    END TRY
    BEGIN CATCH
      ROLLBACK

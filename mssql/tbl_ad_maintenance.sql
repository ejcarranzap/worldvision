...
IF OBJECT_ID('fn_seq_tbl_ad_maintenance') IS NOT NULL DROP FUNCTION fn_seq_tbl_ad_maintenance
IF OBJECT_ID('sp_tbl_ad_maintenance') IS NOT NULL DROP PROCEDURE sp_tbl_ad_maintenance
GO
CREATE FUNCTION fn_seq_tbl_ad_maintenance()
  RETURNS INT
AS
BEGIN
  DECLARE @ID INT = 0
  SELECT @ID = ISNULL(MAX(Id_maintenance),0)+1 FROM tbl_ad_maintenance
  RETURN @ID
END
GO
CREATE PROCEDURE sp_tbl_ad_maintenance
...
SET NOCOUNT ON
  ...
  BEGIN
    ...
    A.user_mod,
    A.active
    FROM tbl_ad_maintenance A WHERE 1 = 1
    AND (COALESCE(@Id_maintenance,A.Id_maintenance) = A.Id_maintenance OR @Id_maintenance IS NULL)
  ...
  BEGIN 
    BEGIN TRY
    BEGIN TRAN
      SET IDENTITY_INSERT tbl_ad_maintenance ON
      
      SET @Id_maintenance = dbo.fn_seq_tbl_ad_maintenance()
      INSERT INTO tbl_ad_maintenance(
      Id_maintenance,
      Id_vehicle,
      Id_supplier,
      Id_maintenance_type,
      ...
      SELECT A.* FROM tbl_ad_maintenance A 
      WHERE A.Id_maintenance = @IDENTITY 
      SET IDENTITY_INSERT tbl_ad_maintenance OFF
    COMMIT
    END TRY
    BEGIN CATCH
      ROLLBACK
  ...
  BEGIN
    DELETE
    FROM tbl_ad_maintenance
    WHERE Id_maintenance = @Id_maintenance
    
    SELECT @Id_maintenance Id_maintenance
  END

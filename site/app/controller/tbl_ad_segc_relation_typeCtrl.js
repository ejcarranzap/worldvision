(function() {
    ...
    app.controller('tbl_ad_segc_relation_typeCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
        $rootScope.Panel = 'Tipo de Relacion (WorldVision)';
        $rootScope.PanelTitle = 'Administracion';
        $rootScope.PanelSubTitle = 'Seguridad Corporativa';
        $rootScope.PanelSubSubTitle = 'Tipo de Relacion (WorldVision)';
        $scope.record = {};
        $scope.data = {};
        $scope.pivot_data = {};
        ...
        $scope.open = function(record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_segc_relation_type_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_segc_relation_type_detCtrl',
                size: size,
        ...
        $log.info('tbl_ad_segc_relation_typeCtrl');
    });

    app.controller('tbl_ad_segc_relation_type_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
        ...
        $scope.save = function() {
            ...
            $scope.urls.push({
                url: '/server/main/tbl_ad_segc_relation_type',
                params: $scope.record,
                action: (!$scope.record.Id_relation_type ? 'I' : 'U')
            });
            autenticationService.call($scope.urls).then(function(results) {
                setTimeout(function() {
        ...
        $log.info('tbl_ad_segc_relation_type_detCtrl');
    });
}());

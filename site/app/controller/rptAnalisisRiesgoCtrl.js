(function() {
    ...
    app.controller('rptAnalisisRiesgoCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
      $rootScope.Panel = 'Analisís de Riesgo';
      $rootScope.PanelTitle = 'Seguridad Corporativa';
      $rootScope.PanelSubTitle = 'Reportes';
      $rootScope.PanelSubSubTitle = 'Analisís de Riesgo';
      $scope.record = {};
      ...
      $scope.getData = function(){
        $scope.urls = [];
        $scope.urls.push({url: '/server/main/tool', params: $scope.record, action: 'H'});
        autenticationService.call($scope.urls).then(function(results){
          setTimeout(function(){
      ...
      $scope.getColumns = function(row, type){
            var keys = Object.keys(row);
            keys.splice(keys.indexOf('$$hashKey'),1);
            if(type == 'E'){
                keys.splice(keys.indexOf('Id_incident'),1);
                keys.splice(keys.indexOf('deta1'),1);
                keys.splice(keys.indexOf('deta2'),1);
                keys.splice(keys.indexOf('deta3'),1);
            }
            return keys;
      };
      ...
      $log.info('rptAnalisisRiesgoCtrl');
    });
  }());

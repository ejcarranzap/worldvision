(function () {
	...
	app.controller('treeLocationAccessCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
		$rootScope.Panel = 'Ubicaciones Acceso (tree)';
		$rootScope.PanelTitle = 'Administración';
		$rootScope.PanelSubTitle = 'Administración';
		$rootScope.PanelSubSubTitle = 'Ubicaciones Acceso (tree)';
		$scope.record = {};
		...
		$scope.onSelectionChange = function (event, data) {
			...
			$scope.mynode = $filter('filter')($scope.data,function(itm){
				return(itm.Id_location==node.Id_location);
			});
		...
		$scope.prepareData = function () {
			angular.forEach($scope.data, function (itm) {
				...
				itm.text = itm.location + '(' + itm.Id_location_type + ')';
		...
		$scope.getData = function () {
			$scope.urls = [];
			$scope.urls.push({
				url: '/server/main/tbl_ad_user_access',
				params: {
			...
			autenticationService.call($scope.urls).then(function (results) {
				setTimeout(function () {
					/*$scope.datapivot = $filter('filter')(results[0].data.data, function(itm){return (parseInt(itm.active)==1)});*/
					$scope.data = results[0].data.data;/*angular.copy($scope.datapivot,$scope.data);*/

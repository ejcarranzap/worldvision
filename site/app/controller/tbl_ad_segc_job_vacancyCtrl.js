(function() {
    ...
    app.controller('tbl_ad_segc_job_vacancyCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
        $rootScope.Panel = 'Puesto';
        $rootScope.PanelTitle = 'Administracion';
        $rootScope.PanelSubTitle = 'Seguridad Corporativa';
        $rootScope.PanelSubSubTitle = 'Puesto';
        $scope.record = {};
        $scope.data = {};
        $scope.pivot_data = {};
        ...
        $scope.open = function(record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_segc_job_vacancy_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_segc_job_vacancy_detCtrl',
                size: size,
        ...
        $log.info('tbl_ad_segc_job_vacancyCtrl');
    });

    app.controller('tbl_ad_segc_job_vacancy_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
        ...
        $scope.save = function() {
            ...
            $scope.urls.push({
                url: '/server/main/tbl_ad_segc_job_vacancy',
                params: $scope.record,
                action: (!$scope.record.Id_job_vacancy ? 'I' : 'U')
            });
            autenticationService.call($scope.urls).then(function(results) {
                setTimeout(function() {
        ...
        $log.info('tbl_ad_segc_job_vacancy_detCtrl');
    });
}());

(function() {
  ...
  app.controller('tbl_ad_batteryCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Batería';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Transacciones';
    $rootScope.PanelSubSubTitle = 'Batería';
    $scope.record = {};
    $scope.data = {};
    $scope.pivot_data = {};
    ...
    $scope.getData = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_battery', params: {}, action: 'S'});
      ...
      $scope.urls.push({url: '/server/main/tbl_ad_battery_make', params: {}, action: 'S'});
      $scope.urls.push({url: '/server/main/tbl_ad_battery_type', params: {}, action: 'S'});
      ...
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
          ...
          $scope.tbl_ad_battery_makes = results[3].data.data;
          $scope.tbl_ad_battery_types = results[4].data.data;
		  $scope.tbl_ad_currencys = results[5].data.data;
          $scope.prepareData();
          $scope.loadData();
          $scope.$apply();
        },0);
    ...
    $scope.prepareData = function () {
      angular.forEach($scope.data, function (itm) {
        ...
        var myitm = [];
        myitm = $filter('filter')($scope.tbl_ad_battery_makes,function(itmx){
          return (itmx.Id_battery_make == itm.Id_battery_make)
        });
        if(myitm)
          if(myitm.length > 0)
            itm.tbl_ad_battery_make = myitm[0].name;
        var myitm = [];
        myitm = $filter('filter')($scope.tbl_ad_battery_types,function(itmx){
          return (itmx.Id_battery_type == itm.Id_battery_type)
        });
        if(myitm)
          if(myitm.length > 0)
            itm.tbl_ad_battery_type = myitm[0].name;
    ...
    $scope.open = function (record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_battery_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_battery_detCtrl',
                size: size,
                ...
                resolve: {
                  ...
                  tbl_ad_battery_makes: function(){return($scope.tbl_ad_battery_makes)},
                  tbl_ad_battery_types: function(){return($scope.tbl_ad_battery_types)},
    ...
    $log.info('tbl_ad_batteryCtrl');
  });

  app.controller('tbl_ad_battery_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
    $uibModalInstance, record, tbl_ad_vehicles, tbl_ad_suppliers, tbl_ad_battery_makes, tbl_ad_battery_types, tbl_ad_currencys) {
    $scope.record = record;
    ...
    $scope.tbl_ad_battery_makes = $filter('filter')(tbl_ad_battery_makes, function(itm){return (parseInt(itm.active)==1)});
    $scope.tbl_ad_battery_types = $filter('filter')(tbl_ad_battery_types, function(itm){return (parseInt(itm.active)==1)});
    ...
    $scope.record.cmbId_battery_make = {};
    $scope.record.cmbId_battery_type = {};
    ...
    $scope.cargaCmb = function(){
        ...
        item = $filter('filter')($scope.tbl_ad_battery_makes, function(itm){
          return (itm.Id_battery_make == $scope.record.Id_battery_make);
        });
        if(item.length > 0){
          $scope.record.cmbId_battery_make = item[0];
        }
        item = $filter('filter')($scope.tbl_ad_battery_types, function(itm){
          return (itm.Id_battery_type == $scope.record.Id_battery_type);
        });
        if(item.length > 0){
          $scope.record.cmbId_battery_type = item[0];
        }
    ...
    $scope.save = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_battery', params: $scope.record, action: (!$scope.record.Id_battery ? 'I' : 'U')});
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
    ...
    $log.info('tbl_ad_battery_detCtrl');
  });
}());

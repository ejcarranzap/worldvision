(function() {
  ...
  app.controller('tbl_ad_process_typeCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Tipo Proceso';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Expediente';
    $rootScope.PanelSubSubTitle = 'Tipo Proceso';
    $scope.record = {};
    $scope.data = {};
    $scope.pivot_data = {};
    ...
    $scope.getData = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_process_type', params: {}, action: 'S'});
    ...
    $scope.open = function (record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_process_type_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_process_type_detCtrl',
                size: size,
    ...
    $log.info('tbl_ad_process_typeCtrl');
  });

  app.controller('tbl_ad_process_type_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
    ...
    $scope.save = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_process_type', params: $scope.record, action: (!$scope.record.Id_process_type ? 'I' : 'U')});
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
    ...
    $log.info('tbl_ad_process_type_detCtrl');
  });
}());

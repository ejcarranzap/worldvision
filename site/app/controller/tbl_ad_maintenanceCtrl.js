(function() {
  ...
  app.controller('tbl_ad_maintenanceCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Mantenimiento';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Transacciones';
    $rootScope.PanelSubSubTitle = 'Mantenimiento';
    $scope.record = {};
    $scope.data = {};
    $scope.pivot_data = {};
    ...
    $scope.getData = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_maintenance', params: {}, action: 'S'});
    ...
    $scope.open = function (record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_maintenance_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_maintenance_detCtrl',
                size: size,
    ...
    $log.info('tbl_ad_maintenanceCtrl');
  });

  app.controller('tbl_ad_maintenance_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
    $uibModalInstance, record, tbl_ad_vehicles, tbl_ad_suppliers, tbl_ad_maintenance_types, tbl_ad_currencys) {
    $scope.record = record;
    ...
    $scope.save = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_maintenance', params: $scope.record, action: (!$scope.record.Id_maintenance ? 'I' : 'U')});
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
    ...
    $log.info('tbl_ad_maintenance_detCtrl');
  });
}());

(function() {
    ...
    app.controller('tbl_ad_segc_news_typeCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
        $rootScope.Panel = 'tbl_ad_segc_news_type';
        $rootScope.PanelTitle = 'Title';
        $rootScope.PanelSubTitle = 'SubTitle';
        $rootScope.PanelSubSubTitle = 'tbl_ad_segc_news_type';
        $scope.record = {};
        $scope.data = {};
        $scope.pivot_data = {};
        ...
        $scope.open = function(record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_segc_news_type_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_segc_news_type_detCtrl',
                size: size,
        ...
        $log.info('tbl_ad_segc_news_typeCtrl');
    });

    app.controller('tbl_ad_segc_news_type_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
        ...
        $scope.save = function() {
            ...
            $scope.urls.push({
                url: '/server/main/tbl_ad_segc_news_type',
                params: $scope.record,
                action: (!$scope.record.Id_news_type ? 'I' : 'U')
            });
            autenticationService.call($scope.urls).then(function(results) {
                setTimeout(function() {
        ...
        $log.info('tbl_ad_segc_news_type_detCtrl');
    });
}());

(function () {
    ...
    app.controller('rptVehicleByDateCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
      $rootScope.Panel = 'Kilometraje Vehículo por Fecha';
      $rootScope.PanelTitle = 'Administración';
      $rootScope.PanelSubTitle = 'Reportes';
      $rootScope.PanelSubSubTitle = 'Kilometraje Vehículo por Fecha';
      $scope.record = {};
      ...
      $scope.getData = function () {
        $scope.urls = [];
        $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'F' });
        autenticationService.call($scope.urls).then(function (results) {
          setTimeout(function () {

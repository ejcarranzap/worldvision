(function () {
	...
	app.controller('treeLocationCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
		$rootScope.Panel = 'Ubicaciones';
		$rootScope.PanelTitle = 'Administración';
		$rootScope.PanelSubTitle = 'Administración';
		$rootScope.PanelSubSubTitle = 'Ubicaciones';
		$scope.record = {};
		...
		$scope.prepareData = function () {
			angular.forEach($scope.data, function (itm) {
				...
				itm.text = itm.Id_location + ' - ' + itm.name + '(' + itm.Id_location_type + ')';
		...
		$scope.getData = function () {
			...
			$scope.urls.push({
				url: '/server/main/tbl_ad_location_type',
				params: {},
				action: 'S'
			});
			autenticationService.call($scope.urls).then(function (results) {
				setTimeout(function () {
					...
					$scope.tbl_ad_locations = angular.copy($scope.data, $scope.tbl_ad_locations);
					$scope.tbl_ad_location_types = results[1].data.data;
					$scope.prepareData();
		...
		$scope.save = function () {
			...
			$scope.urls.push({
				url: '/server/main/tbl_ad_location',
				params: $scope.record,
				action: (!$scope.record.Id_location ? 'I' : 'U')
			});
			autenticationService.call($scope.urls).then(function (results) {
				setTimeout(function () {
					$scope.record = {};
		...
		$scope.cargaCmb = function(){
			...
			item = $filter('filter')($scope.tbl_ad_locations, function(itm){
			  return (itm.Id_location == $scope.record.Id_location_parent);
			});
			if(item.length > 0){
			  $scope.record.cmbId_location_parent = item.slice(0)[0];
			}
		};

		$log.info('treeLocationCtrl');
	});
}());
(function() {
  ...
  app.controller('tbl_ad_scheduling_stateCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Estado Reservación';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Administración';
    $rootScope.PanelSubSubTitle = 'Estado Reservación';
    $scope.record = {};
    $scope.data = {};
    $scope.pivot_data = {};
    ...
    $scope.open = function (record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_scheduling_state_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_scheduling_state_detCtrl',
                size: size,
    ...
    $log.info('tbl_ad_scheduling_stateCtrl');
  });

  app.controller('tbl_ad_scheduling_state_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
    ...
    $scope.save = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_scheduling_state', params: $scope.record, action: (!$scope.record.Id_scheduling_state ? 'I' : 'U')});
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
    ...
    $log.info('tbl_ad_scheduling_state_detCtrl');
  });
}());

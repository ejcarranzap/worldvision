(function() {
  ...
  app.controller('tbl_ad_fuel_typeCtrl', function($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Tipo de Combustible';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Vehículo';
    $rootScope.PanelSubSubTitle = 'Tipo de Combustible';
    $scope.record = {};
    $scope.data = {};
    $scope.pivot_data = {};
    ...
    $scope.open = function (record, size, actn) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/view/tbl_ad_fuel_type_det.html?dt=' + (new Date()).getTime(),
                controller: 'tbl_ad_fuel_type_detCtrl',
                size: size,
    ...
    $log.info('tbl_ad_fuel_typeCtrl');
  });

  app.controller('tbl_ad_fuel_type_detCtrl', function(Upload, $timeout, $scope, $rootScope, $log, $window, $http, $filter, autenticationService,
    ...
    $scope.save = function(){
      $scope.urls = [];
      $scope.urls.push({url: '/server/main/tbl_ad_fuel_type', params: $scope.record, action: (!$scope.record.Id_fuel_type ? 'I' : 'U')});
      autenticationService.call($scope.urls).then(function(results){
        setTimeout(function(){
    ...
    $log.info('tbl_ad_fuel_type_detCtrl');
  });
}());

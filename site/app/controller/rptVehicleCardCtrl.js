(function () {
    ...
    app.controller('rptVehicleCardCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
      $rootScope.Panel = 'Ficha Vehículo';
      $rootScope.PanelTitle = 'Administración';
      $rootScope.PanelSubTitle = 'Reportes';
      $rootScope.PanelSubSubTitle = 'Ficha Vehículo';
      $scope.record = {};
      ...
      $scope.getData = function () {
        $scope.urls = [];
        $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'G' });
        $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'C' });
        $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'B' });
        $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'A' });
        $scope.urls.push({url: '/server/main/tbl_ad_tire', params: {Id_vehicle: $scope.record.Id_vehicle}, action: 'S'});
        $scope.urls.push({url: '/server/main/tbl_ad_battery', params: {Id_vehicle: $scope.record.Id_vehicle}, action: 'S'});
        $scope.urls.push({url: '/server/main/tbl_ad_new', params: {Id_vehicle: $scope.record.Id_vehicle}, action: 'S'});
        autenticationService.call($scope.urls).then(function (results) {
          setTimeout(function () {
            $scope.data = results[0].data.data;
            $scope.dataa = $scope.data[0];
            $scope.datab = results[1].data.data;
            $scope.datac = results[2].data.data;
            $scope.datad = results[3].data.data;
            $scope.datae = results[4].data.data;
            $scope.dataf = results[5].data.data;
            $scope.datag = results[6].data.data;
            $scope.prepareData();
            $scope.loadData();
            $scope.$apply();
          }, 0);

(function () {
  ...
  app.controller('rptVehicleLocationCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
    $rootScope.Panel = 'Vehículos por Ubicación';
    $rootScope.PanelTitle = 'Administración';
    $rootScope.PanelSubTitle = 'Reportes';
    $rootScope.PanelSubSubTitle = 'Vehículos por Ubicación';
    $scope.record = {};
    ...
    $scope.getData = function () {
      $scope.urls = [];
      $scope.urls.push({ url: '/server/main/tool', params: $scope.record, action: 'E' });
      autenticationService.call($scope.urls).then(function (results) {
        setTimeout(function () {
    ...
    $log.info('rptVehicleLocationCtrl');
  });
}());

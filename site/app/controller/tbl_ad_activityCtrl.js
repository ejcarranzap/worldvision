(function () {
	...
	app.controller('tbl_ad_activityCtrl', function ($scope, $rootScope, $log, $window, $http, $filter, autenticationService, $uibModal) {
		$rootScope.Panel = 'Actividad';
		$rootScope.PanelTitle = 'Expediente';
		...
		$scope.prepareData = function () {
			angular.forEach($scope.data, function (itm) {
				itm.id = itm.Id_activity;
				itm.text = itm.Id_activity+ ' - ' + itm.code + ' - ' +itm.name + '(' + itm.Id_activity_parent + ')';
				itm.parent = itm.Id_activity_parent;
		...
		$scope.getData = function () {
			$scope.urls = [];
			$scope.urls.push({
				url: '/server/main/tbl_ad_activity',
				params: {},
				action: 'S'
			});

			$scope.urls.push({
				url: '/server/main/tbl_ad_process_type',
				params: {},
				action: 'S'
			});
			...
			autenticationService.call($scope.urls).then(function (results) {
				setTimeout(function () {
					...
					$scope.data2 = results[1].data.data;

					$scope.tbl_ad_activity = angular.copy($scope.data, $scope.tbl_ad_activity);
		...
		$scope.save = function () {
			...
			$scope.urls.push({
				url: '/server/main/tbl_ad_activity',
				params: $scope.record,
				action: (!$scope.record.Id_activity ? 'I' : 'U')
			});
			autenticationService.call($scope.urls).then(function (results) {
				setTimeout(function () {
					$scope.record = {};
		...
		$scope.cargaCmb = function(){
			...
			item = $filter('filter')($scope.tbl_ad_activity, function(itm){
			  return (itm.Id_activity == $scope.record.Id_activity_parent);
			});
			if(item.length > 0){
			  $scope.record.cmbId_activity_parent = item.slice(0)[0];
			}

			item = $filter('filter')($scope.tbl_ad_process_type, function(itm){
				return (itm.Id_process_type == $scope.record.Id_process_type);
			  });
			  if(item.length > 0){
				$scope.record.cmbId_process_type = item.slice(0)[0];
			  }
		};

		$log.info('tbl_ad_activityCtrl');
	});
}());
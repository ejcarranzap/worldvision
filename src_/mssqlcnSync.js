module.exports.mssqlcnSync = {
  ...
  execute: function(request, reply, calls, callback, config){
    try{
          /*console.log(module.exports.dbsql);*/
                  ...
                  if(validateError(results) != false){
                    if(call.deta){
                      for(var ii = 0; ii < call.deta.length; ii++){
                          ...
                          if(results)
                          if(results[0])
                          if(results[0].recordsets)
                          if(results[0].recordsets[0])
                          if(results[0].recordsets[0].length > 0){
                              parent = results[0].recordsets[0][0];
                          }
                          /*console.log('PARENT RECORD');
                          console.log(parent);
                          console.log(calld.paramsToReplace);*/
